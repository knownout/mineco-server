...
$queryInitialString = "update materials set";
$queryList = [];
foreach ($content as $key => $value) $queryList[] = "$key='$value'";
if ($content["pinned"] === "1") $database->query("update materials set pinned=0");

$queryString = $queryInitialString . " " . join(", ", $queryList) . " where identifier='$identifier'";
...
@file_put_contents($textFilePath, json_encode($text, JSON_UNESCAPED_SLASHES + JSON_UNESCAPED_UNICODE + JSON_PRETTY_PRINT));
